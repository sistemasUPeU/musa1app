...
export class VinculacionBusComponent implements OnInit {
  ...
  @ViewChild("modalVinculacion", { static: false })
  modalVinculacion: ElementRef;
  ...
  registrarVinculacionBus() {
    ...
    this.vinculacionService
      .registrarVinculacionBus(this.vinculacion_modal)
      .subscribe(data => {
        let id = data;
        ...
        for (let i = 0; i < this.vreqs_modal.length; i++) {
          this.vinculacionService
            .registrarVinculacionRequisito(this.vreqs_modal[i], id)
            .subscribe(data1 => {
  ...
  cargarElementosVinculacionBus(id: number) {
    this.vinculacionService
      .cargarElementosVinculacionBus(id)
      .subscribe(data => {
  ...
  actualizarEstadoRequisito(pos: number) {
    if (
      this.vreqs_modal[pos].url === null ||
      this.vreqs_modal[pos].url === ""
    ) {
  ...
  accionBuscarVinculacion() {
    ...
    console.log("*contador (igual a lenght, no se encontro):");
    console.log(i);
    console.log("*tamanio del arreglo: ");
    console.log(this.vinculaciones.length);
